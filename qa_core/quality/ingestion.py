...
def build_ingestion_quality_report(
    *,
    ...
    执行流程：
      1. 解析业务场景并构建 DataScope，把 tenant/dataset/visibility/roles 写入 metadata。
    ...
    for path in candidate_files:
        ...
        if spec is None:
            #不通过时，先检查这个文件有没有风险图片-》即使不支持loader，也需要记录图片风险
            image_risk = analyze_image_risk(path, "")
            if image_risk:
                payload = image_risk.as_dict()
                payload["source"] = source
                image_risk_files.append(payload)
            # 记录到unsupported_files，包括路径、后缀、拒绝原因
        ...
        # 加载文件，标准化metadata、切分成chunk，但是不写入milvus，只在内存中做
        result = _process_candidate_file(path, source, scenario, active_kb_version, scope, allowed_roles)
        # 三道门完成以后，按照多个维度分桶收集
        # 每个桶对应了一个质量维度，后续在警告和门禁阈值分别检查不同的桶，比如：failed_files对应max_failed_files阈值
    ...
    # 检测faq标准答案和正文资料的潜在冲突，分为两步：
    #  1：先找相关正文，FAQ问题和答案一起用jieb分词，过滤单字与停用词，按照顺序去重，最多保留12个关键字，同source的正文命中数量达到动态门槛后才进入候选
    #  2：在对比事实口径，同时检查数字集合是否完全无交集，以及肯定/否定倾向，是否相反等。

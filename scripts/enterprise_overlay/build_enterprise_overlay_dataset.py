...
def ensure_generated_dir(path: Path, build_root: Path) -> None:
    ...
    调用顺序：命令行入口 -> ensure_generated_dir()。
    """
...
def build_single_overlay_dataset(scenario_id: str, pack_root: Path, build_root: Path) -> dict[str, Any]:
    ...
    调用顺序：命令行入口 -> build_single_overlay_dataset()。
    """
    ...
    # ---------- 步骤 1：合并文档资料 ----------
    # 先复制基础场景文档，再复制 overlay 增强文档（同名文件会被覆盖并记录）
    ...
    # ---------- 步骤 2：合并 FAQ ----------
    # 基础 FAQ 优先，overlay 中同名问题被标记为 duplicate 并跳过（不静默覆盖）
    ...
    # ---------- 步骤 3：入库质量检查 ----------
    # 合并后的数据集通过质量报告验证，确保增强资料不会引入解析失败等问题
...
def resolve_requested_scenarios(args: argparse.Namespace) -> list[str]:
    ...
    调用顺序：命令行入口 -> resolve_requested_scenarios()。
    """

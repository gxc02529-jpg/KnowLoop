...
def collect_markdown_files(input_dir: Path) -> list[Path]:
    ...
    调用顺序：命令行入口 -> collect_markdown_files()。
    """
...
def promote_files(args: argparse.Namespace) -> dict[str, Any]:
    ...
    report = {
        ...
        "next_commands": [
            ...
            "python scripts/quality/check_evaluation_gate.py --dataset eval_sets/business_depth_regression.json --limit 32 --min-source-inference-accuracy 1.0 --min-prompt-profile-accuracy 0.85",
        ],
        "rows": rows,
    }

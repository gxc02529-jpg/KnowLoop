...
#   - check_chapter_maps.py       — 校验章节动画节点与代码文件对齐
#
...
CHAPTERS: list[dict[str, object]] = [
    {
        ...
        "desc": "从用户问题进入系统开始，先用低成本规则完成直答、边界保护和检索意图分类。",
        "nodes": [
    ...
    },
    {
        ...
        "nodes": [
            ...
            ("FILTER", "过滤表达式", "qa_core/retrieval/filters.py", "validate_source_filter() / build_source_expr()", "入口校验 source，检索层只构建过滤表达式。"),
    ...
    },
    {
        ...
        "nodes": [
            ...
            ("VALIDATE", "source 校验", "qa_core/pipeline/steps.py", "decide_route()", "入口阶段调用 validate_source_filter() 校验 source_filter。"),
    ...
    },
    {
        ...
        "nodes": [
            ("APP", "应用入口", "app.py", "app =", "创建 FastAPI 应用并注册路由。"),
    ...
    },
    {
        ...
        "desc": "启动前检查必需配置和运行依赖，初始化 MySQL schema，并确认 active 版本可用。",
        "nodes": [
            ...
            ("PREFLIGHT", "基础前置校验", "qa_core/config/preflight.py", "validate_runtime_environment()", "校验占位值、路径、TCP 连接和 LLM 连通性。"),
            ("SCHEMA", "Schema Bootstrap", "qa_core/storage/bootstrap.py", "bootstrap_mysql_schema()", "执行 runtime_schema.sql，一次性初始化 MySQL 表结构。"),
            ("ACTIVE", "Active 版本校验", "qa_core/config/preflight.py", "validate_active_kb_versions()", "schema 就绪后确认当前场景有 active 知识库版本。"),
            ("VALIDATE", "启动守卫", "app.py", "warmup_runtime()", "任一阶段失败直接抛错，阻止启动。"),
            ("APP", "应用接入", "app.py", "warmup_runtime()", "启动事件串联 preflight、schema、active、BERT 预热和后台检索预热。"),
    ...
    },
    {
        ...
        "nodes": [
            ...
            ("API", "版本 API", "qa_core/api/kb_versions.py", "list/rollback/archive payload", "提供轻量版本管理接口。"),
    ...
    },
    {
        ...
        "nodes": [
            ("INGEST", "入库编排", "qa_core/indexing/service.py", "ingest_directory()", "串联场景、数据域、版本、加载、引用式复用、切分和写入。"),
            ...
            ("REUSE", "引用式复用", "qa_core/indexing/service.py", "_reference_base_version()", "未变化 chunk 不复制，目标 manifest 直接引用；修改/删除时由 expire_documents_for_version() 写 valid_to_seq。"),
            ...
            ("WRITE", "写入 store", "qa_core/indexing/service.py", "store.delete_ids() / add_documents()", "新增/修改文件写入新 chunk；修改/删除文件让旧 chunk 从目标版本开始失效。"),
            ("FAQ", "FAQ 入库", "qa_core/indexing/faq_ingestion.py", "faq_documents_from_csv() / ingest_faq_csv()", "把 FAQ CSV 写入 FAQ store。"),
            ("VERSION", "版本记录", "qa_core/governance/kb_versions.py", "record_ingest_result()", "记录入库统计；发布激活由重建脚本门禁收口。"),
        ],
    },
    {
        "no": "17",
...
BUSINESS_NODES_BY_CHAPTER: dict[str, list[tuple[str, str, str, str]]] = {
    ...
    "13": [
        ...
        ("B_CHECK", "逐项前置校验", "环境检查", "检查占位值、必需路径、active scenario 和外部依赖连通性。"),
        ("B_SCHEMA", "初始化 MySQL Schema", "表结构入口", "统一执行 runtime_schema.sql，业务 Store 不再按需建表。"),
        ("B_ACTIVE", "校验 active 知识库版本", "业务状态检查", "表结构就绪后确认当前场景有可检索版本。"),
    ...
    "16": [
        ...
        ("B_MANIFEST", "对比增量清单", "版本构建", "用 MySQL IndexManifest 判断目标版本跳过、基准版本引用或变化重建。"),
        ("B_REUSE", "引用未变化文档", "跨版本增量", "未变化 chunk 不复制，目标版本 manifest 继续引用旧 chunk。"),
        ...
        ("B_WRITE", "写入检索 Store", "入库闭环", "变化文件让旧 chunk 失效后写入新 chunk；未变化文件由有效期视图继续可见。"),
        ("B_VERSION", "记录版本统计", "版本治理", "记录 reembedded/reused/skipped 数量；质量门禁通过后由发布脚本激活新版本。"),
        ("B_OUT", "上线可检索", "本章交付", "线上按 active version_seq 解释有效版本视图。"),
    ],
    "17": [
